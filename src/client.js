window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Resolve toast background/foreground colors by type.
		 * @param {"success"|"error"|"warning"} type
		 * @returns {object} style object with bg/fg
		 */
		function toastItemStyle(type) {
			var colors = {
				success: { bg: "var(--dsw-alias-state-success-primary)", fg: "var(--dsw-alias-label-primary-foreground, #fff)" },
				error: { bg: "var(--dsw-alias-label-error)", fg: "var(--dsw-alias-label-primary-foreground, #fff)" },
				warning: { bg: "var(--dsw-alias-accent, #f59e0b)", fg: "var(--dsw-alias-label-primary-foreground, #fff)" },
			};
			var c = colors[type] || colors.success;
			return Object.assign({}, toastItemBaseStyle, { background: c.bg, color: c.fg });
		}

		/**
		 * Toast notification component.
		 * Renders a fixed-position stack of auto-dismissing notifications.
		 * Supports success (green), error (red), and warning (amber) types.
		 *
		 * @param {object} props
		 * @param {Array<{id: string, message: string, type: string}>} props.toasts - Active toast items
		 * @param {function(string): void} props.onDismiss - Called with toast id when clicked
		 * @returns {JSX.Element|null} Toast container or null if empty
		 */
			...
			return j("div", {
				...
				children: toasts.map(function(toast) {
					return j("div", {
						...
						children: [
							j("span", { key: "i", style: { fontSize: "14px", flexShrink: 0 }, children: icons[toast.type] || "✓" }),
							j("span", { key: "m", style: { flex: 1 }, children: toast.message }),
						],
					});
				}),
			});
		...
		const popoverTriggerVisibleStyle = Object.assign({}, popoverTriggerStyle, {
			opacity: 1,
		...
		// Popover menu open state animation – applied after mount
		const popoverMenuOpenStyle = {
			opacity: 1,
		...
		/**
		 * PopoverMenu — hover-triggered action menu.
		 *
		 * A "⋯" button appears on hover (or when `visible` is true). Clicking it
		 * opens a dropdown menu with action items. Closes on outside click/Escape.
		 *
		 * @param {object} props
		 * @param {Array<{icon: string, label: string, onClick: function, danger?: boolean}>} props.items - Menu action items
		 * @param {boolean} props.visible - Force the trigger button to be visible
		 * @returns {JSX.Element}
		 */
		function PopoverMenu({ items, visible }) {
			var _useState = react.useState;
			var openState = _useState(false), setOpen = openState[1], open = openState[0];
			var _useRef = react.useRef;
			var _useEffect = react.useEffect;
			var containerRef = _useRef(null);

			// Animate menu open – apply the open style after a tick
			var _useState2 = react.useState;
			var animState = _useState2(false), setAnim = animState[1], anim = animState[0];
			_useEffect(function() {
				if (open) {
					var raf = requestAnimationFrame(function() { setAnim(true); });
			...
			return js("div", {
				...
				children: [
					j("button", {
						key: "trigger",
						style: visible || open ? popoverTriggerVisibleStyle : popoverTriggerStyle,
					...
					open ? j("div", {
						key: "menu",
						style: anim ? Object.assign({}, popoverMenuStyle, popoverMenuOpenStyle) : popoverMenuStyle,
						children: items.map(function(item, i) {
							return j("button", {
								key: i,
								style: item.danger ? Object.assign({}, popoverItemStyle, { color: "var(--dsw-alias-label-error)" }) : popoverItemStyle,
								onClick: function(e) {
									e.stopPropagation();
									setOpen(false);
								...
								children: [
									j("span", { key: "i", children: item.icon || "" }),
									j("span", { key: "l", children: item.label }),
								],
							});
						}),
					}) : null,
		...
		// Backdrop open state – applied after mount
		const modalBackdropOpenStyle = {
			opacity: 1,
		};
		...
		// Card open state – applied after mount
		const modalCardOpenStyle = {
			opacity: 1,
			transform: "translateY(0) scale(1)",
		};
		 ...
		 *
		 * Renders a fixed overlay with backdrop blur. Closes on Escape key or
		 * backdrop click. Supports an optional footer row.
		 *
		 * Fade-in animation: backdrop and card start at opacity 0 and animate in
		 * via requestAnimationFrame after mount.
		 *
		 * @param {object} props
		 * @param {string} props.title - Modal title bar text
		 * @param {function(): void} props.onClose - Close callback (Escape/backdrop)
		 * @param {JSX.Element} props.children - Modal body content
		 * @param {JSX.Element} [props.footer] - Optional footer (e.g. action buttons)
		 * @param {string} [props.width] - Optional explicit width override
		 * @returns {JSX.Element}
		 */
		function Modal({ title, onClose, children, footer, width }) {
			var _useEffect = react.useEffect;
			var _useState = react.useState;
			var animState = _useState(false), setAnim = animState[1], anim = animState[0];
			...
			return j("div", {
				style: anim ? Object.assign({}, modalBackdropStyle, modalBackdropOpenStyle) : modalBackdropStyle,
				onClick: onBackdropClick,
				role: "dialog",
				"aria-modal": "true",
				children: j("div", {
					style: anim ? Object.assign({}, modalCardStyle, modalCardOpenStyle) : modalCardStyle,
		...
		const en = {
			...
			recallHint: "Enter keywords to test memory recall",
			// Settings modal
		...
		const categoryMeta = {
			coding: { icon: "💻", labelKey: "categoryCoding", fg: "var(--dsw-alias-accent, #60a5fa)", bg: "rgba(96,165,250,0.12)", border: "rgba(96,165,250,0.35)" },
			communication: { icon: "💬", labelKey: "categoryCommunication", fg: "var(--dsw-alias-state-success-primary, #4ade80)", bg: "rgba(74,222,128,0.12)", border: "rgba(74,222,128,0.35)" },
			workflow: { icon: "⚙️", labelKey: "categoryWorkflow", fg: "var(--dsw-alias-accent, #fb923c)", bg: "rgba(251,146,60,0.12)", border: "rgba(251,146,60,0.35)" },
			safety: { icon: "🛡", labelKey: "categorySafety", fg: "var(--dsw-alias-label-error, #f87171)", bg: "rgba(248,113,113,0.12)", border: "rgba(248,113,113,0.35)" },
		};

		// Memory types — long-memory extended (USER/PREFERENCE/PROJECT/FACT/SKILL/EVENT/TASK)
		const memoryTypeMeta = {
			USER:       { icon: "👤", labelKey: "memTypeUser",       fg: "var(--dsw-alias-accent, #c084fc)", bg: "rgba(192,132,252,0.12)", border: "rgba(192,132,252,0.35)" },
			PREFERENCE: { icon: "❤️", labelKey: "memTypePreference", fg: "var(--dsw-alias-label-error, #fb7185)", bg: "rgba(251,113,133,0.12)", border: "rgba(251,113,133,0.35)" },
			PROJECT:    { icon: "📁", labelKey: "memTypeProject",    fg: "var(--dsw-alias-accent, #fbbf24)", bg: "rgba(251,191,36,0.12)",  border: "rgba(251,191,36,0.35)" },
			FACT:       { icon: "📌", labelKey: "memTypeFact",       fg: "var(--dsw-alias-accent, #60a5fa)", bg: "rgba(96,165,250,0.12)",  border: "rgba(96,165,250,0.35)" },
			SKILL:      { icon: "🛠", labelKey: "memTypeSkill",      fg: "var(--dsw-alias-state-success-primary, #4ade80)", bg: "rgba(74,222,128,0.12)",  border: "rgba(74,222,128,0.35)" },
			EVENT:      { icon: "📅", labelKey: "memTypeEvent",      fg: "var(--dsw-alias-accent, #a78bfa)", bg: "rgba(167,139,250,0.12)", border: "rgba(167,139,250,0.35)" },
			TASK:       { icon: "✅", labelKey: "memTypeTask",       fg: "var(--dsw-alias-state-success-primary, #34d399)", bg: "rgba(52,211,153,0.12)",  border: "rgba(52,211,153,0.35)" },
		};
		...
		// ─── CorrectionsTab ───────────────────────────────────────────────────
		function CorrectionsTab({ t, badges, setBadges, highlightId, onClearHighlight, showToast }) {
			var _useState = react.useState;
			...
			_useEffect(function() {
				fetch('/plugins/dsh-long-memory/api/stats')
					...
					.catch(function() { if (showToast) showToast(t("loadFailed"), "error"); });
			}, []);
			...
			function doExtract(correction) {
				...
				fetch('/plugins/dsh-long-memory/api/corrections/' + encodeURIComponent(correction.id) + '/extract', { method: 'POST' })
					...
					.catch(function() { setActiveOp(null); if (showToast) showToast(t("loadFailed"), "error"); });
			}
				...
				if ((rule.hit_count || 0) > 20) {
					buttons.push(j("button", {
						...
						style: {
							...
							padding: "0 14px",
							background: "transparent",
							color: "var(--dsw-alias-accent, #fbbf24)",
		...
		// ─── RulesTab ─────────────────────────────────────────────────────────
		function RulesTab({ t, badges, setBadges, onJumpToCorrection, showToast }) {
			var _useState = react.useState;
			var _useEffect = react.useEffect;
			...
			function handleApprove(rule) {
				...
				fetch('/plugins/dsh-long-memory/api/rules/' + encodeURIComponent(rule.id) + '/approve', { method: 'POST' })
					...
					.catch(function() { setBusyId(null); if (showToast) showToast(t("loadFailed"), "error"); })
		...
		// ─── MemoryCard ───────────────────────────────────────────────────────
		/**
		 * MemoryCard — displays a single long-memory entry.
		 *
		 * Shows type badge, scope, content, confidence bar, origin, access count,
		 * and tags. Archived/superseded memories are visually dimmed. Hover reveals
		 * a PopoverMenu with archive/delete actions.
		 *
		 * @param {object} props
		 * @param {object} props.memory - Memory record (type, scope, content, tags, confidence, origin, access_count, status)
		 * @param {function(string): string} props.t - i18n translate function
		 * @param {function(object): void} props.onArchive - Archive callback
		 * @param {function(object, boolean, string): void} props.onDelete - Delete callback (memory, hard, reason)
		 * @param {string|null} props.busy - ID of the memory currently being operated on (disables actions)
		 * @param {function(string, string): void} props.showToast - Toast notification callback
		 * @returns {JSX.Element}
		 */
		function MemoryCard({ memory, t, onArchive, onDelete, busy, showToast }) {
			var _useState = react.useState;
			var hoveredState = _useState(false), setHovered = hoveredState[1], hovered = hoveredState[0];
			var typeMeta = memoryTypeMeta[memory.type] || { icon: "⚪", labelKey: null, fg: "var(--dsw-alias-label-tertiary, #94a3b8)", bg: "rgba(148,163,184,0.12)", border: "rgba(148,163,184,0.35)" };
			...
			function handleArchive() {
				if (typeof window !== "undefined" && window.confirm && !window.confirm(t("memArchiveConfirm"))) return;
				if (onArchive) onArchive(memory);
			}
			...
			return js("div", {
				onMouseEnter: function() { if (!isArchived) setHovered(true); },
				...
				children: [
					...
					// Actions — PopoverMenu (archive / delete)
					!isArchived ? js("div", { key: "act", style: { position: "absolute", top: "8px", right: "12px" }, children: [
						j(PopoverMenu, {
							key: "pm",
							visible: hovered,
							items: [
								{ icon: "📦", label: t("memArchive"), onClick: handleArchive },
								{ icon: "🗑", label: t("memDelete"), onClick: handleDelete, danger: true },
							],
						}),
					]}) : null,
				]
		...
		function MemoriesTab({ t, badges, setBadges, activeTab, showToast, scopeFilter, searchQuery }) {
			var _useState = react.useState;
			var _useEffect = react.useEffect;
			...
			var scopeFilterState = _useState(scopeFilter || "all"), setScopeFilter = scopeFilterState[1], scopeFilter = scopeFilterState[0];
			var searchInputState = _useState(searchQuery || ""), setSearchInput = searchInputState[1], searchInput = searchInputState[0];
			var activeSearchState = _useState(searchQuery || ""), setActiveSearch = activeSearchState[1], activeSearch = activeSearchState[0];
			var busyIdState = _useState(null), setBusyId = busyIdState[1], busyId = busyIdState[0];
			...
			function doDelete(memory, hard, reason) {
				...
				.then(function() {
					...
					loadMemories();
					refreshBadges();
					if (showToast) showToast(t("memDeleteSuccess"), "success");
				})
				.catch(function() { setBusyId(null); if (showToast) showToast(t("loadFailed"), "error"); });
			}
			...
			function doArchive(memory) {
				setBusyId(memory.id);
				fetch('/plugins/dsh-long-memory/api/memories/' + encodeURIComponent(memory.id), {
					method: 'PUT',
					headers: { 'Content-Type': 'application/json' },
				...
				.then(function() {
					...
					loadMemories();
					refreshBadges();
					if (showToast) showToast(t("memArchiveSuccess"), "success");
				})
				.catch(function() { setBusyId(null); if (showToast) showToast(t("loadFailed"), "error"); });
			}
		...
		function PersonaTab({ t, badges, setBadges, activeTab, showToast }) {
			var _useState = react.useState;
			...
			function doRebuild() {
				...
				fetch('/plugins/dsh-long-memory/api/persona/rebuild', { method: 'POST' })
					...
					.catch(function() { setRebuilding(false); if (showToast) showToast(t("loadFailed"), "error"); });
			}
		...
		// ─── LongMemorySidebarEntry (sidebar button + floating modal) ────────
		/**
		 * LongMemorySidebarEntry — sidebar trigger button that opens the full
		 * long-memory settings panel in a floating modal overlay.
		 *
		 * When the sidebar is collapsed (wide=false), shows only the icon.
		 * When expanded (wide=true), shows icon + label text. The modal
		 * contains the LongMemorySettingsTab with all memory management tabs.
		 *
		 * @param {object} props
		 * @param {function(string): string} props.t - i18n translate function
		 * @param {boolean} props.wide - Whether the sidebar is in expanded mode
		 * @returns {JSX.Element}
		 */
		function LongMemorySidebarEntry({ t, wide }) {
			var _useState = react.useState;
			var modalOpenState = _useState(false), setModalOpen = modalOpenState[1], modalOpen = modalOpenState[0];
			var hoverState = _useState(false), setHover = hoverState[1], hovered = hoverState[0];

			return js("div", { children: [
				// Sidebar button — matches settings button style
				j("button", {
					...
					children: [
						wide ? j(primitives.IconDataOutline16, { key: "icon", size: 16 }) : j(primitives.IconDataOutline16, { key: "icon", size: 18 }),
						wide ? j("span", { key: "label", style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, children: t("tab") }) : null,
					],
				}),
				// Floating modal
				modalOpen ? js("div", {
					...
					onClick: function(e) { if (e.target === e.currentTarget) setModalOpen(false); },
					children: [
						js("div", {
							style: {
								position: "relative",
							...
							children: [
								...
								j("button", {
									...
									style: {
										...
										zIndex: 1, lineHeight: 1, padding: "4px",
									},
									children: "×",
								}),
								j(LongMemorySettingsTab, { t: t }),
							],
						}),
					],
				}) : null,
		...
		function LongMemorySettingsTab({ t }) {
			...
			var toastsState = _useState([]), setToasts = toastsState[1], toasts = toastsState[0];
			var toastIdRef = _useRef(0);
			...
			// M1: Sidebar state
			var sidebarWidthState = _useState(220), setSidebarWidth = sidebarWidthState[1], sidebarWidth = sidebarWidthState[0];
			var selectedScopeState = _useState("all"), setSelectedScope = selectedScopeState[1], selectedScope = selectedScopeState[0];
			var searchQueryState = _useState(""), setSearchQuery = searchQueryState[1], searchQuery = searchQueryState[0];
			var scopeCountsState = _useState({ all: 0, user: 0, project: 0, domain: 0, episodic: 0 }), setScopeCounts = scopeCountsState[1], scopeCounts = scopeCountsState[0];
			var scopeCountsLoadingState = _useState(true), setScopeCountsLoading = scopeCountsLoadingState[1], scopeCountsLoading = scopeCountsLoadingState[0];
			// M2: Settings modal + recall test
			var settingsModalOpenState = _useState(false), setSettingsModalOpen = settingsModalOpenState[1], settingsModalOpen = settingsModalOpenState[0];
			var recallQueryState = _useState(""), setRecallQuery = recallQueryState[1], recallQuery = recallQueryState[0];
			var recallResultsState = _useState(null), setRecallResults = recallResultsState[1], recallResults = recallResultsState[0];
			var recallLoadingState = _useState(false), setRecallLoading = recallLoadingState[1], recallLoading = recallLoadingState[0];
			var recallErrorState = _useState(null), setRecallError = recallErrorState[1], recallError = recallErrorState[0];
			// Resize ref
			var isResizingRef = _useRef(false);
			...
			function refreshStatsAndBadges() {
				fetch('/plugins/dsh-long-memory/api/stats')
					...
					.catch(function() { showToast(t("loadFailed"), "error"); });
			}
			...
			// Load /api/config once on first open of the settings modal
			...
			// M1: Fetch scope counts for sidebar — uses /api/stats + /api/memories fallback
			function fetchScopeCounts() {
				setScopeCountsLoading(true);
				// Try /api/stats first (same endpoint as refreshStatsAndBadges)
				fetch('/plugins/dsh-long-memory/api/stats')
					.then(function(r) { return r.json(); })
					.then(function(data) {
						if (!data) {
						...
						var total = (data.memories_active || 0) + (data.memories_archived || 0);
						var sc = {
							all: total || (data.memories_active || 0),
							user: data.memories_user || data.user || data.scope_user || data.user_count || 0,
							project: data.memories_project || data.project || data.scope_project || data.project_count || 0,
							domain: data.memories_domain || data.domain || data.scope_domain || data.domain_count || 0,
							episodic: data.memories_episodic || data.episodic || data.scope_episodic || data.episodic_count || 0,
						};
						setScopeCounts(sc);
						// If all per-scope counts are 0, try fetching from /api/memories as fallback
						if (sc.user === 0 && sc.project === 0 && sc.domain === 0 && sc.episodic === 0) {
							fetch('/plugins/dsh-long-memory/api/memories?limit=200')
								.then(function(r2) { return r2.json(); })
								.then(function(memData) {
									var arr = Array.isArray(memData) ? memData : (memData.rows || memData.items || memData.memories || []);
									var counts = { all: arr.length, user: 0, project: 0, domain: 0, episodic: 0 };
									arr.forEach(function(m) {
										var s = m.scope;
										if (s === 'user') counts.user++;
										else if (s === 'project') counts.project++;
										else if (s === 'domain') counts.domain++;
										else if (s === 'episodic') counts.episodic++;
									});
									setScopeCounts(counts);
									setScopeCountsLoading(false);
								})
								.catch(function() { showToast(t("loadFailed"), "error"); setScopeCountsLoading(false); });
						} else {
							setScopeCountsLoading(false);
						}
					})
			...
			_useEffect(function() {
				...
				function onMouseMove(e) {
					...
					rafId = requestAnimationFrame(function() {
						rafId = null;
						setSidebarWidth(Math.max(160, Math.min(360, e.clientX)));
					});
				}
				function onMouseUp() {
			...
			// M2: Recall test handler
			function handleRecallTest() {
				var q = recallQuery.trim();
				if (!q) return;
				setRecallLoading(true);
				setRecallResults(null);
				setRecallError(null);
				fetch('/plugins/dsh-long-memory/api/memories?q=' + encodeURIComponent(q) + '&limit=20')
					.then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
					.then(function(data) {
						setRecallResults(Array.isArray(data) ? data : (data.rows || data.items || data.memories || []));
						setRecallLoading(false);
					})
					.catch(function(e) {
						setRecallLoading(false);
						setRecallError((e && e.message) || t("loadFailed"));
			...
			var tabs = [
				...
				{ id: "recall", label: t("recallTab"), badge: 0 },
			];

			var scopeItems = [
				{ id: "all", i18n: "sidebarScopeAll" },
				{ id: "user", i18n: "sidebarScopeUser" },
				{ id: "project", i18n: "sidebarScopeProject" },
				{ id: "domain", i18n: "sidebarScopeDomain" },
				{ id: "episodic", i18n: "sidebarScopeEpisodic" },
			];
			return js("div", { style: { display: "flex", height: "100%" }, children: [
				// ── Sidebar ──
				js("div", { style: { width: sidebarWidth, overflowY: "auto", borderRight: "1px solid var(--dsw-alias-border-l2)", flexShrink: 0, background: "var(--dsw-alias-bg-layer-1)", display: "flex", flexDirection: "column" }, children: [
					// Search box
					js("div", { style: { padding: "12px" }, children: [
						j("input", {
							type: "text",
							value: searchQuery,
							...
							style: Object.assign({}, inputStyle, { height: "32px", fontSize: "12px", borderRadius: "6px" }),
						}),
					]}),
					// Scope buttons
					js("div", { style: { padding: "0 12px 8px", display: "flex", flexDirection: "column", gap: "2px" }, children: scopeItems.map(function(sc) {
						var isActive = selectedScope === sc.id;
						var count = scopeCounts[sc.id] || 0;
						return js("button", {
							key: sc.id,
							onClick: function() { setSelectedScope(sc.id); setActiveTab("memories"); },
							style: {
								display: "flex", justifyContent: "space-between", alignItems: "center",
								padding: "6px 10px", border: "none", borderRadius: "6px",
								background: isActive ? "var(--dsw-alias-bg-layer-2)" : "transparent",
								color: isActive ? "var(--dsw-alias-label-primary)" : "var(--dsw-alias-label-secondary)",
								fontSize: "12px", cursor: "pointer", width: "100%",
								lineHeight: "18px", transition: "background 0.15s",
							},
							children: [
								j("span", { key: "label", children: t(sc.i18n) }),
								scopeCountsLoading ? j("span", { key: "loading", style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" }, children: "\u2026" }) :
								j("span", { key: "count", style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" }, children: count }),
							],
						});
					})}),
					// Separator
					j("div", { style: { margin: "4px 12px", borderTop: "1px solid var(--dsw-alias-border-l2)" } }),
					// Stats
					js("div", { style: { padding: "8px 12px", fontSize: "11px", color: "var(--dsw-alias-label-tertiary)", display: "flex", flexDirection: "column", gap: "4px" }, children: [
						js("div", { style: { display: "flex", justifyContent: "space-between" }, children: [
							j("span", { children: t("sidebarStatsMemories") }),
							j("span", { style: { fontWeight: 600, color: "var(--dsw-alias-label-primary)" }, children: scopeCounts.all || 0 }),
						]}),
						js("div", { style: { display: "flex", justifyContent: "space-between" }, children: [
							j("span", { children: t("sidebarStatsActive") }),
							j("span", { style: { fontWeight: 600, color: "var(--dsw-alias-state-success-primary)" }, children: stats ? stats.memories_active : 0 }),
						]}),
					]}),
				]}),
				// Resize handle
				j("div", { onMouseDown: startResize, style: { width: 4, cursor: "col-resize", flexShrink: 0, background: "var(--dsw-alias-border-l2)", transition: "background 0.15s" } }),
				// Main content
				js("div", { style: { flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }, children: [
					// ── Header: title + config + ON/OFF ──
					js("div", { style: { padding: "16px 20px 0", borderBottom: "1px solid var(--dsw-alias-border-l2)" }, children: [
						js("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "6px" }, children: [
							...
							js("div", { style: { display: "flex", alignItems: "center", gap: "10px" }, children: [
								j("button", {
									...
									onClick: function() { setSettingsModalOpen(true); },
								...
								j("button", {
									...
									style: {
										...
										color: enabled ? "var(--dsw-alias-label-primary-foreground, #fff)" : "var(--dsw-alias-label-tertiary)",
						...
						// Overview strip (pending lessons / pending rules / active memories)
						js("div", {
							style: {
								padding: "8px 20px",
					...
					js("div", {
						...
						children: activeTab === "lessons" ? j(CorrectionsTab, {
							t: t, badges: badges, setBadges: setBadges,
							highlightId: highlightId, showToast: showToast,
							onClearHighlight: function() { setHighlightId(null); },
						}) :
						activeTab === "rules" ? j(RulesTab, {
							t: t, badges: badges, setBadges: setBadges, showToast: showToast,
							onJumpToCorrection: function(cid) { setActiveTab("lessons"); setHighlightId(cid); },
						}) :
						activeTab === "memories" ? j(MemoriesTab, {
							t: t, badges: badges, setBadges: setBadges,
							activeTab: activeTab, showToast: showToast,
							scopeFilter: selectedScope !== "all" ? selectedScope : undefined,
							searchQuery: searchQuery || undefined,
						}) :
						activeTab === "persona" ? j(PersonaTab, {
							t: t, badges: badges, setBadges: setBadges, activeTab: activeTab, showToast: showToast,
						}) :
						// Recall test panel
						js("div", { children: [
							j("h3", { style: Object.assign({}, headerStyle, { fontSize: "16px", marginBottom: "12px" }), children: t("recallTab") }),
							j("p", { style: { fontSize: "13px", color: "var(--dsw-alias-label-secondary)", marginBottom: "16px" }, children: t("recallHint") }),
							js("div", { style: { display: "flex", gap: "8px", marginBottom: "16px" }, children: [
								j("input", {
									type: "text", value: recallQuery,
									placeholder: t("recallSearchPlaceholder"),
									onChange: function(e) { setRecallQuery(e.target.value); },
									onKeyDown: function(e) { if (e.key === "Enter") handleRecallTest(); },
									style: Object.assign({}, inputStyle, { flex: 1, height: "36px" }),
								}),
								j("button", {
									onClick: handleRecallTest,
									disabled: recallLoading || !recallQuery.trim(),
									style: Object.assign({}, btnPrimary, { height: "36px", fontSize: "13px", opacity: recallLoading ? 0.6 : 1 }),
									children: recallLoading ? t("recallSearching") : t("recallSearch"),
								}),
							]}),
							recallLoading ? j(SkeletonCard, { count: 3 }) :
							recallError ? j("div", {
								style: { textAlign: "center", padding: "32px 20px", color: "var(--dsw-alias-label-error)", fontSize: "13px" },
								children: recallError,
							}) :
							recallResults && recallResults.length === 0 ? j("div", {
								style: { textAlign: "center", padding: "32px 20px", color: "var(--dsw-alias-label-tertiary)", fontSize: "13px" },
								children: t("recallNoResults"),
							}) :
							recallResults ? recallResults.map(function(m, i) {
								return js("div", {
									key: m.id || i,
									style: Object.assign({}, cardStyle, { padding: "12px 16px", marginBottom: "8px" }),
									children: [
										js("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }, children: [
											j("span", { key: "sc", style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" }, children: t("recallScore") + ": " + (m.score !== undefined ? Number(m.score).toFixed(4) : "—") }),
											j("span", { key: "so", style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" }, children: t("recallSource") + ": " + ((m.scope || m.source || "—")) }),
										]}),
										j("p", { key: "ct", style: { fontSize: "13px", color: "var(--dsw-alias-label-primary)", margin: 0, lineHeight: 1.5, wordBreak: "break-word" }, children: (m.content || m.text || m.summary || "—").substring(0, 300) }),
									],
								});
							}) : null,
						]}),
					...
					settingsModalOpen ? (function() {
						...
						return j(Modal, {
							...
							onClose: function() { setSettingsModalOpen(false); },
							width: "620px",
							children: js("div", { style: { display: "flex", flexDirection: "column", gap: "12px", maxHeight: "60vh", overflowY: "auto", padding: "4px 0" }, children: [
								// Card 1: Extraction
								js("div", { style: Object.assign({}, cardStyle, { padding: "12px 16px" }), children: [
									...
									js("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }, children: [
										js("div", { children: [
											...
											j("input", { type: "number", min: 1, max: 100, style: inputStyle, value: draft.batchSize, onChange: function(e) { updateDraft({ batchSize: Number(e.target.value) }); } }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("promoteThreshold") }),
											j("input", { type: "number", min: 1, max: 100, style: inputStyle, value: draft.ruleThreshold, onChange: function(e) { updateDraft({ ruleThreshold: Number(e.target.value) }); } }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("llmTimeout") + " (" + t("llmTimeoutUnit") + ")" }),
											j("input", { type: "number", min: 1000, step: 1000, style: inputStyle, value: draft.llmTimeoutMs, onChange: function(e) { updateDraft({ llmTimeoutMs: Number(e.target.value) }); } }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("modelSelect") }),
											j("input", { type: "text", placeholder: t("modelFollowCurrent"), style: inputStyle, value: draft.model || '', onChange: function(e) { updateDraft({ model: e.target.value }); } }),
										]}),
									]}),
								]}),
								...
								js("div", { style: Object.assign({}, cardStyle, { padding: "12px 16px" }), children: [
									...
									js("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }, children: [
										js("div", { children: [
											j("div", { style: labelStyle, children: t("personaFreqSessions", {n: draft.personaEverySessions}) }),
											j("input", { type: "number", min: 1, max: 10000, style: inputStyle, value: draft.personaEverySessions, onChange: function(e) { updateDraft({ personaEverySessions: Number(e.target.value) }); } }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("personaFreqDays", {n: Math.round(draft.personaEveryMs / 86400000)}) }),
											j("input", { type: "number", min: 1, max: 365, style: inputStyle, value: Math.round(draft.personaEveryMs / 86400000), onChange: function(e) { var days = Math.max(1, Number(e.target.value) || 1); updateDraft({ personaEveryMs: days * 86400000 }); } }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("embedding") }),
											j("select", { style: selectStyle, value: embedding.autoDetect === false ? "none" : "auto", onChange: function(e) { updateDraftEmbedding({ autoDetect: e.target.value !== "none" }); }, children: [j("option", { value: "auto", children: t("embeddingAuto") }), j("option", { value: "none", children: t("embeddingNone") })] }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("embeddingModelLabel") }),
											j("input", { type: "text", style: inputStyle, value: embedding.preferredModel || 'bge-m3', onChange: function(e) { updateDraftEmbedding({ preferredModel: e.target.value }); } }),
										]}),
										js("div", { style: { gridColumn: "1 / span 2" }, children: [
											j("div", { style: labelStyle, children: t("embeddingBaseUrlLabel") }),
											j("input", { type: "text", style: inputStyle, value: embedding.ollamaBaseUrl || 'http://127.0.0.1:11434', onChange: function(e) { updateDraftEmbedding({ ollamaBaseUrl: e.target.value }); } }),
										]}),
										js("div", { children: [
											j("div", { style: labelStyle, children: t("embeddingTimeoutLabel") + " (" + t("llmTimeoutUnit") + ")" }),
											j("input", { type: "number", min: 100, step: 100, style: inputStyle, value: embedding.timeoutMs || 1000, onChange: function(e) { updateDraftEmbedding({ timeoutMs: Number(e.target.value) }); } }),
										]}),
										js("div", { style: { display: "flex", alignItems: "flex-end" }, children: [
											j("button", { style: Object.assign({}, btnOutline, { height: "36px", fontSize: "12px", opacity: testingEmbedding ? 0.6 : 1 }), disabled: testingEmbedding, onClick: testEmbedding, children: testingEmbedding ? t("embeddingTesting") : t("embeddingTest") }),
										]}),
									]}),
								]}),
								// Signal words
								js("div", { children: [
									...
									j("textarea", { rows: 2, placeholder: t("signalWordsPlaceholder"), style: Object.assign({}, inputStyle, { height: "auto", padding: "8px 12px", resize: "vertical", fontFamily: "inherit" }), value: draft.signalWordsText || '', onChange: function(e) { updateDraft({ signalWordsText: e.target.value }); } }),
									j("div", { style: descStyle, children: t("signalWordsHint") }),
								]}),
								// Status line
								js("div", { style: { fontSize: "12px", color: statusLineColor }, children: [
									saveMsg ? saveMsg.text : ([t("embeddingStatus"), "：", j("span", { style: { color: embStatusColor, fontWeight: 500 }, children: renderEmbeddingStatus() })]),
								]}),
							]}),
							footer: js("div", { style: { display: "flex", justifyContent: "flex-end", gap: "8px" }, children: [
								j("button", { style: Object.assign({}, btnOutline, { height: "32px", fontSize: "12px" }), onClick: function() { setSettingsModalOpen(false); }, children: t("cancel") }),
								j("button", { style: Object.assign({}, btnPrimary, { height: "32px", fontSize: "12px", opacity: savingConfig ? 0.6 : 1 }), disabled: savingConfig, onClick: function() { saveConfig(); }, children: savingConfig ? t("savingConfig") : t("saveConfig") }),
							]}),
						});
					})() : null,
					// ── Toast notifications ──
					j(Toast, { key: "toast", toasts: toasts, onDismiss: dismissToast }),
				]}),
			]});
		}
		...
		/**
		 * i18n-safe translate wrapper.
		 * Returns the translation if available; otherwise falls back to the
		 * key name itself (prefixed with "[missing:" in dev, bare key in prod)
		 * so the UI never crashes on a missing translation key.
		 *
		 * @param {function(string): string} t - Original locale-bound translate
		 * @returns {function(string): string} Safe translate function
		 */
		function safeT(t) {
			return function(key) {
				var result = t(key);
				// If the result is the key itself, the locale system didn't find it
				if (result === key || typeof result === "undefined" || result === "") {
					return "[" + key + "]";
				}
				return result;
			};
		...
		function apply(ctx) {
			...
			const t = safeT(ctx.locale.bind(NS));
