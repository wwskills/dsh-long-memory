...
import { matchSignalWords, resolveSignalWords } from './corrections.js';
import {
...
function createLongMemory(ctx, config) {
    ...
    try {
      ...
      try {
        ...
        if (matchSignalWords(text, signals)) {
          ...
          if (recent && String(recent.error_summary || '').startsWith(prefix)) {
  ...
  ctx.on('tools/result', (exec, result) => {
    try {
      const isError = !result || result.status === 'error' || result.status === 'failed' || result.isFailure === true || (result.status !== 'success' && typeof result.error === 'string' && result.error.trim().length > 0) || result.ok === false;
      if (!isError) return;
  ...
  ctx.on('agent/error', (payload) => {
    try {
      const err = payload?.error; const error_summary = String(err?.message || (typeof err === 'string' ? err : 'agent error')).slice(0, 240);
