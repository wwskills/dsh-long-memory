...
import {
  ...
  countPendingCorrections, listRules, getRule, updateRule, approveRule, rejectRule, promoteRule,
...
import { TaskQueue, streamLlm, buildLessonPrompt, buildRulePrompt, parseJsonResponse } from './extract.js';
import { matchSignalWords, resolveSignalWords, extractUserText as aeExtractUserText, DEFAULT_SIGNAL_WORDS } from './corrections.js';
import {
...
function createLongMemory(ctx, config) {
  ...
  ctx.on('session/event', (_session, event) => {
    try {
      ...
      // ── agent-evolve merge: signal word detection ──
      try {
        const signals = DEFAULT_SIGNAL_WORDS;
      ...
      } catch (e2) { console.warn('[long-memory] signal word detection failed:', e2?.message); }
    } catch (e) {
  ...
  ctx.on('tools/result', (exec, result) => {
    try {
      const isError = !result || result.status === 'error' || result.status === 'failed' || result.isFailure === true || (typeof result.error === 'string' && result.error.trim().length > 0) || result.ok === false;
      if (!isError) return;
      const error_summary = String(result?.error || result?.message || result?.reason || (exec?.name || 'tool') + ': error').slice(0, 240);
      insertCorrection(state.driver, { trigger: 'tool_error', error_summary, context: JSON.stringify({ tool: exec?.name }), sessionId: exec?.sessionId || 'unknown' });
    } catch (e) { console.warn('[long-memory] tools/result handler failed:', e?.message || e); }
  }, { global: true });

  ctx.on('agent/error', (payload) => {
    try {
      const error_summary = String(payload?.error?.message || payload?.error || 'agent error').slice(0, 240);
    ...
    } catch (e) { console.warn('[long-memory] agent/error handler failed:', e?.message || e); }
  }, { global: true });
      ...
      try {
        ...
        if (rules && rules.length > 0) {
          ...
          if (hitIds.length > 0) {
            ...
            for (const id of hitIds) { try { incrementRuleHit(state.driver, id); } catch {} }
          }
        }
      } catch (e) { console.warn('[long-memory] rule injection failed:', e?.message || e); }
  ...
  const rulesDecayInterval = setInterval(() => {
    try { archiveStaleRules(state.driver); } catch {}
  }, 24 * 60 * 60 * 1000);
  ctx.effect(() => { clearInterval(rulesDecayInterval); return () => {}; }, 'long-memory: rules decay');
  ...
  const registerWebRoutes = () => {
    ...
    ctx.effect(() => ws.register({ kind: 'exact', path: AE + '/api/corrections', handler: async (req, res) => {
      try {
        ...
        const url = new URL(req.url || '/', 'http://x');
        const rows = listCorrections(state.driver, { status: url.searchParams.get('status') || undefined, trigger: url.searchParams.get('trigger') || undefined, limit: Math.min(parseInt(url.searchParams.get('limit') || '200'), 500) });
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' });
    ...
    ctx.effect(() => ws.register({ kind: 'prefix', path: AE + '/api/corrections/', handler: async (req, res) => {
      try {
        const url = new URL(req.url || '/', 'http://x');
        const m = url.pathname.match(/\/api\/corrections\/([^/]+)\/(extract|ignore)$/);
    ...
    ctx.effect(() => ws.register({ kind: 'exact', path: AE + '/api/rules', handler: async (req, res) => {
      try {
        if (req.method !== 'GET') { res.writeHead(405); res.end(); return; }
        const url = new URL(req.url || '/', 'http://x');
        const rows = listRules(state.driver, { status: url.searchParams.get('status') || undefined, limit: Math.min(parseInt(url.searchParams.get('limit') || '100'), 500) });
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' });
        res.end(JSON.stringify(rows));
      } catch (e) { res.writeHead(500); res.end(JSON.stringify({ error: e?.message })); }
    }}));
    ...
    ctx.effect(() => ws.register({ kind: 'prefix', path: AE + '/api/rules/', handler: async (req, res) => {
      try {
        const url = new URL(req.url || '/', 'http://x');
        const am = url.pathname.match(/\/api\/rules\/([^/]+)\/(approve|reject|promote|source)$/);
        if (am) {
          ...
          if (action === 'source') { const rule = getRule(state.driver, id); if (!rule) { res.writeHead(404); res.end(); return; } const sids = JSON.parse(rule.source_corrections || '[]'); const corrs = sids.map(sid => getCorrection(state.driver, sid)).filter(Boolean); res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ rule, corrections: corrs })); return; }
        }
        const im = url.pathname.match(/\/api\/rules\/([^/]+)$/);
        if (im && req.method === 'PUT') {
          const body = await readRequestBody(req); const patch = JSON.parse(body);
          const ok = updateRule(state.driver, im[1], { content: patch.content, category: patch.category, tags: Array.isArray(patch.tags) ? patch.tags : (patch.tags ? String(patch.tags).split(',').map(s => s.trim()).filter(Boolean) : undefined) });
    ...
    ctx.effect(() => ws.register({ kind: 'exact', path: AE + '/api/persona', handler: async (req, res) => {
      try {
        ...
        for (const r of rows) {
          ...
          const m = r.content.match(/^([a-z_]+)\s*[:：]\s*(.+)$/i);
          if (m) {
            persona[m[1].toLowerCase()] = { value: m[2].trim(), confidence: r.confidence, updated_at: r.observed_at };
          } else {
            // Use content as-is with auto key
            persona['field_' + r.id.slice(0, 8)] = { value: r.content, confidence: r.confidence, updated_at: r.observed_at };
          }
          if (r.observed_at > lastUpdated) lastUpdated = r.observed_at;
        }
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify({ persona, last_updated_at: lastUpdated || null }));
      } catch (e) { res.writeHead(500); res.end(JSON.stringify({ error: e?.message })); }
    }}));
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'GET') {
            ...
            if (state.settingsHandle) {
              try {
                state.settingsHandle.update({ embedding: patch });
                saved = true;
              } catch (e) {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    try {
      ...
      // After registration, read the resolved config (base + user overrides)
...
/** Shallow merge for config objects (persisted overrides cfg). */
...
function pickBaseFromConfig(cfg) {
  ...
  for (const [dotted, path] of Object.entries(mapping)) {
    const v = readPath(cfg, Array.isArray(path) ? path.join('.') : path);
