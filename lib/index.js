...
function createLongMemory(ctx, config) {
  ...
  const state = {
    ...
    embeddingConfig: readEmbeddingConfigFile(),
    activeScope: 'project'
  };
  ...
  const registerWebRoutes = () => {
    ...
    ctx.effect(() => ws.register({
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'GET') {
            // Priority: file config (user-saved) > settings namespace (patch.yml defaults)
            const fileConfig = readEmbeddingConfigFile();
            let current = {};
            try {
              const ns = ctx.settings.get('long-memory');
              current = ns?.embedding || {};
            } catch (e) { /* settings not available */ }
            // If file has a non-none provider, use it (user saved via UI)
            const merged = fileConfig.provider !== 'none' ? fileConfig : {
            ...
            // Try settings service first; fall back to file if ctx.settings not injected
            try {
              await ctx.settings.update('long-memory', { embedding: patch });
            } catch (settingsErr) {
              // ctx.settings not available in this fiber — write to file directly
              writeEmbeddingConfigFile(patch);
            }
...
function memSearch(svc) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      // M3: hot-reload embedding config from file so Web UI changes take
      // effect immediately (no dsh restart required).
      const fileConfig = readEmbeddingConfigFile();
      const liveConfig = fileConfig.provider !== 'none' ? fileConfig : svc.cfg.embedding;
...
function embeddingConfigPath() {
  const home = process.env.DSH_HOME || (process.env.HOME + '/.dsh');
  return home + '/long-memory/config.json';
}

function readEmbeddingConfigFile() {
  try {
    const data = JSON.parse(fs.readFileSync(embeddingConfigPath(), 'utf8'));
    return {
      provider: data.provider || 'none',
      model: data.model || '',
      dimension: data.dimension || 1024,
      batch_size: data.batch_size || 16,
      timeout_ms: data.timeout_ms || 30000,
      ollama: { base_url: data.ollama?.base_url || 'http://127.0.0.1:11434' },
      openai_compatible: { base_url: data.openai_compatible?.base_url || '', api_key: data.openai_compatible?.api_key || '' }
    };
  } catch {
    return {
      provider: 'none', model: '', dimension: 1024, batch_size: 16, timeout_ms: 30000,
...
function writeEmbeddingConfigFile(patch) {
  const fp = embeddingConfigPath();
  fs.mkdirSync(path.dirname(fp), { recursive: true });
  fs.writeFileSync(fp, JSON.stringify(patch, null, 2), 'utf8');
}
