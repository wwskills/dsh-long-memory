...
function createLongMemory(ctx, config) {
  ...
  const registerWebRoutes = () => {
    ...
    ctx.effect(() => ws.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          sendJsonError(res, e?.statusCode && e.statusCode >= 400 ? e.statusCode : 500, e?.message || 'internal error');
        }
      }
    ...
    ctx.effect(() => ws.register({ kind: 'prefix', path: AE + '/api/rules/', handler: async (req, res) => {
      ...
      } catch (e) { sendJsonError(res, e?.statusCode && e.statusCode >= 400 ? e.statusCode : 500, e?.message || 'internal error'); }
    }}));
        ...
        try {
          if (req.method === 'GET') {
            ...
            if (!patch || typeof patch !== 'object' || Array.isArray(patch)
              || (patch.provider !== undefined && !PROVIDERS.includes(patch.provider))) {
...
function applySettingsToCfg(cfg, resolved) {
  ...
  for (const [dotted, path] of Object.entries(SETTINGS_CFG_MAPPING)) {
    ...
    if (v === undefined) continue;
    writePath(cfg, Array.isArray(path) ? path.join('.') : path, v);
  }
}
