...
export function recallFts5(driver, args) {
  ...
  // Bump access counters (best-effort; do not abort on failure). The
  // vector-enriched re-run passes skipAccessBump so a single mem_search call
...
export function computeVectorSimilarity(driver, queryVec, filter = {}) {
  ...
  // Scan cap for the pre-filtered candidate list.
  const scanLimit = filter.limit ?? 5000;
  // Pre-filter memories, then look up cached embeddings per row (hash-based,
  // the same key writeMemory uses).
  const memSql = `
  ...
  for (const mem of allMemories) {
    ...
    // Filter by dim (and model when known): the embeddings table is keyed by
    // (content_sha256, model, dim), and without the dim filter a cache row
    // from a previous embedding model would be served for the same content —
    // cosine against a mismatched dimension silently scores 0 for everything.
