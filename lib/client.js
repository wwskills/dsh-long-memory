window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function embSummary(emb) {
			if (!emb || emb.provider === 'none') return 'FTS5 only';
		...
		function MemorySettingsTab({ t }) {
			...
			function doSaveEmb() {
				...
				fetch('/plugins/dsh-long-memory/api/embedding-config', {
					...
					if (r.ok) { setEmbMsg('✓ Saved. Restart dsh to apply.'); setEmbEdit(false); setEmbSnapshot(null); }
			...
			if (!emb) {
				...
				embSection = js(react.Fragment, { children: [
					...
					}), "none = FTS5 关键词降级; ollama = 本地服务; openai-compatible = 云 API"),
					...
					j("div", { style: descStyle, children: "Changes take effect on dsh restart." }),
				]});
			}
			...
			return js("div", { style: { padding: "16px 0", display: "flex", flexDirection: "column", gap: "16px" }, children: [
				...
				js("div", { style: cardStyle, children: [
					...
					js("div", { style: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px" }, children: [
						statCard("Embedding", embLabel, "var(--dsw-alias-label-secondary)"),
						statCard("Memories", total > 0 ? String(total) : "0", "var(--dsw-alias-label-success)"),
						statCard("Pending", queue ? String(queue.length) : "0", "var(--dsw-alias-label-secondary)"),
					]}),
				]}),
				...
				js("div", { style: cardStyle, children: [
					...
					j("div", { style: headerDescStyle, children: "配置向量召回的 Embedding provider。修改后需重启 DSH 生效。" }),
					j("div", { style: dividerStyle }),
					embSection,
				]}),
			]});
