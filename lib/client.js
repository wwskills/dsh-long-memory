window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			providerNoneDesc: t('providerNoneDesc'),
			modelHint: t('modelHint'),
			dimensionHint: t('dimensionHint'),
			batchSizeHint: t('batchSizeHint'),
			timeoutHint: t('timeoutHint'),
			edit: "编辑",
			cancel: "取消",
			saving: "保存中...",
			save: "保存",
		...
		function MemorySettingsTab({ t }) {
			...
			function doSaveEmb() {
				...
				fetch('/plugins/dsh-long-memory/api/embedding-config', {
					...
					if (r.ok) { setEmbMsg(t('savedMsg')); setEmbEdit(false); setEmbSnapshot(null); }
				...
				embSection = js(react.Fragment, { children: [
					...
					js("div", { style: { display: "flex", alignItems: "center", gap: "8px", justifyContent: "flex-end" }, children: [
						...
						j("button", {
							...
							children: embSavingFlag ? t('saving') : t('save')
						}),
					]}),
			...
			return js("div", { style: { padding: "16px 0", display: "flex", flexDirection: "column", gap: "16px" }, children: [
				...
				js("div", { style: cardStyle, children: [
					...
					j("div", { style: headerDescStyle, children: t('embeddingDesc') }),
					j("div", { style: dividerStyle }),
					embSection,
				]}),
			]});
