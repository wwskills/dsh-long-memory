...
export async function embedBatch(driver, config, texts) {
  ...
  for (let i = 0; i < fetched.length; i++) {
    ...
    // Store the ACTUAL vector length as dim — a configured dimension that the
    // provider does not honour must not be recorded as if it did, or later
    // dim-filtered cache lookups serve mismatched vectors.
    driver.prepare(
...
export function parseEmbedding(blob) {
  if (!blob) return null;
  ...
  // honor byteOffset/byteLength instead of decoding the whole underlying
  // buffer. A length that is not a multiple of 4 is corrupt — skip it.
  const byteLength = blob.byteLength ?? blob.length;
