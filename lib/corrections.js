...
export function archiveStaleRules(db, ttlMs = 90 * 86400000) {
  ...
  // Rules still being injected refresh last_hit_at and legitimately stay alive;
  // rules that were NEVER injected must still expire — fall back to created_at
  // for them (the old `last_hit_at IS NOT NULL` filter made them immortal).
  const r = db.prepare(
...
export function getRulesForInjection(db, { category, limit = 20 } = {}) {
  ...
  // rules with equal hit_count (and stops pure hit-count rich-get-richer).
